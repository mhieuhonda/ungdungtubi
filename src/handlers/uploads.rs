...
pub async fn upload_info() -> HttpResponse {
...
pub async fn upload_image(
    ...
    while let Some(Ok(mut field)) = payload.next().await {
        field_count += 1;
        if field_count > 5 {
            // Chống spam nhiều field
            return HttpResponse::BadRequest().json(serde_json::json!({
                "error": "Quá nhiều field trong form upload."
            }));
        }

        let cd_opt = field.content_disposition().cloned();
        let field_name = cd_opt
            .as_ref()
            .and_then(|cd| cd.get_name())
            .unwrap_or("")
            .to_string();
        let field_filename = cd_opt
            .as_ref()
        ...
        if let Some(fname) = field_filename {
            // Sanitize filename — chỉ giữ lại alphanumeric + . + _ + -
        ...
        // Đọc bytes, kiểm tra size
        while let Some(Ok(chunk)) = field.next().await {
            file_bytes.extend_from_slice(&chunk);
            if file_bytes.len() as u64 > config.max_upload_bytes as u64 {
...
/// Hỗ trợ: PNG, JPEG, GIF, WebP (đơn giản).
...
/// PNG: bytes 16..24 = width (BE) + height (BE)
...
/// GIF: bytes 6..10 = width (LE) + height (LE) (2 bytes each)
...
/// JPEG: scan các marker SOFx để tìm width/height.
fn parse_jpeg_dimensions(bytes: &[u8]) -> (Option<i32>, Option<i32>) {
    ...
    while i + 9 < bytes.len() {
        ...
        // SOF markers: 0xC0..=0xCF (trừ 0xC4 DHT, 0xC8 JPG, 0xCC DAC)
...
/// WebP: RIFF....WEBP, sau đó xem chunk VP8 / VP8L / VP8X
fn parse_webp_dimensions(bytes: &[u8]) -> (Option<i32>, Option<i32>) {
    ...
    match chunk_type {
        b"VP8 " => {
            // Lossy: width/height ở bytes 26..30 (LE, 14-bit each)
            if bytes.len() < 30 {
                return (None, None);
            }
        ...
        b"VP8L" => {
            // Lossless: width/height stored as 14-bit values, signature byte at offset 20
            if bytes.len() < 25 {
                return (None, None);
            }
        ...
        b"VP8X" => {
            // Extended: canvas size at bytes 24..30 (24-bit LE, +1 each)
            if bytes.len() < 30 {
                return (None, None);
            }
