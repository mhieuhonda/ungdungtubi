//! Handlers cho Live Chat real-time trong Nhóm (v0.9.2 — Giai đoạn 7).
//!
//! Bao gồm:
//!   * GET  /ws/cong-dong/nhom/{slug}                       — WebSocket upgrade (auth)
//!   * GET  /api/cong-dong/nhom/{slug}/chat-history         — Lấy 50 tin nhắn gần nhất
//!
    ...
    while let Some(msg_result) = receiver.next().await {
        ...
        if let axum::extract::ws::Message::Text(text) = msg {
            ...
            if body.is_empty() || body.chars().count() > MAX_CHAT_BODY_CHARS {
                // Bỏ qua tin nhắn rỗng hoặc quá dài — gửi error JSON cho client
                let err_payload = serde_json::json!({
                    "type": "error",
                    "message": format!("Tin nhắn không hợp lệ (tối đa {MAX_CHAT_BODY_CHARS} ký tự).")
                })
                .to_string();
                let _ = chat_hub.broadcast(group_id, err_payload).await;
                continue;
            }
